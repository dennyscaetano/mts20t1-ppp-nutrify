...
describe('Casos funcionais negativos extras (pt-BR)', function () {
  ...
  it('retorna 400 ao enviar nome como número no registro', async () => {
    const res = await request(app).post('/users/register').send({ name: 123, email: 'a@b.com', password: '123456' });
    expect(res.status).to.equal(400);
  });

  it('retorna 400 ao criar alimento com calories nulo', async () => {
    await request(app).post('/users/register').send({ name: 'A', email: 'a2@b.com', password: '123456' });
  ...
  it('retorna 401 ao acessar rota protegida com token inválido', async () => {
    const res = await request(app).post('/foods').set('Authorization', 'Bearer invalid.token').send({ name: 'Z', calories: 10 });
    expect(res.status).to.equal(401);
  });

  it('retorna 400 ao atualizar alimento com campo extra inesperado', async () => {
    await request(app).post('/users/register').send({ name: 'C', email: 'c@c.com', password: '123456' });
