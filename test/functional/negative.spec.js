...
describe('Testes funcionais negativos / validação', function () {
  ...
  describe('Validação de usuários e erros de autenticação', function () {
    it('retorna 400 ao registrar sem senha', async () => {
      const res = await request(app).post('/users/register').send({ name: 'NoPass', email: 'nopass@example.com' });
      expect(res.status).to.equal(400);
      expect(res.body).to.have.property('errors').that.is.an('array');
    });

  it('retorna 400 ao registrar com nome vazio', async () => {
      const res = await request(app).post('/users/register').send({ name: '', email: 'blank@example.com', password: '123456' });
      expect(res.status).to.equal(400);
      expect(res.body).to.have.property('errors');
    });

  it('retorna 401 ao autenticar com credenciais incorretas', async () => {
      await request(app).post('/users/register').send({ name: 'Login', email: 'login@example.com', password: 'correct' });
  ...
  describe('Validação de alimentos e autenticação', function () {
    it('retorna 401 ao criar alimento sem token', async () => {
      const res = await request(app).post('/foods').send({ name: 'NoAuth', calories: 10 });
      expect(res.status).to.equal(401);
      expect(res.body).to.have.property('error');
    });

  it('retorna 400 ao criar alimento sem nome', async () => {
      await request(app).post('/users/register').send({ name: 'F', email: 'f2@example.com', password: '123456' });
  ...
  it('retorna 400 quando calories tem tipo errado', async () => {
      await request(app).post('/users/register').send({ name: 'F', email: 'f3@example.com', password: '123456' });
  ...
  describe('Validação de refeições e erros de negócio', function () {
    it('retorna 400 ao criar refeição sem lista de alimentos', async () => {
      await request(app).post('/users/register').send({ name: 'M', email: 'm2@example.com', password: '123456' });
  ...
  it('retorna 400 quando foods não é um array', async () => {
      await request(app).post('/users/register').send({ name: 'M', email: 'm3@example.com', password: '123456' });
  ...
  it('retorna 400 quando refeição referencia id de alimento inexistente', async () => {
      await request(app).post('/users/register').send({ name: 'M', email: 'm4@example.com', password: '123456' });
